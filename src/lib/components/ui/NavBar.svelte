...
<div class="fixed top-0 w-24 left-0 h-screen bg-gray-800 flex flex-col text-white shadow-md z-50">
<Logo/>
<div class="grow flex flex-col justify-between items-center">
  <div>
    {#each d as item}
      <a sveltekit:prefetch class="w-24 h-24 block flex flex-col justify-center items-center gap-2" href={item.lien}>
        <i class="{item.icone} fa-2x text-white"></i>
        <p class="text-xs">{item.texte}</p>
      </a>
    {/each}
  </div>
